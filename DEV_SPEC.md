...
##### 自动化模型组合策略
- **自动生成策略组合**：系统自动识别并生成所有可能的模型组合，无需手动选择
  - 单模型策略：仅包含Benchmark模型（用于与双模型策略对比）
  - 双模型组合：自动生成所有Tier 2（本地小模型）× Tier 3（API大模型）的组合
  - 根据测试集中的`expected_complexity`标签自动路由：
    - 简单询问（simple）→ 使用本地小模型
    - 复杂询问（complex）→ 使用API大模型
  - 实现位置：`src/observability/dashboard/pages/llm_arena.py` 的 `_render_benchmark_run()` 函数

##### 动态进度显示和排行榜
- **实时进度展示**：
  - 进度条显示当前完成的任务数/总任务数
  - 状态文本显示当前正在评估的策略名称
  - 阶段文本显示当前处理阶段（回答生成中、RAGAS评估中等）
  - 动态更新排行榜，每完成一个策略后立即更新
- **实现位置**：`_run_benchmark()` 函数中使用 `st.progress()`、`st.markdown()` 和 `st.info()` 实现

##### 进度保存和恢复功能
- **自动保存进度**：
  - 每次测试用例完成后自动保存到 `logs/benchmark_progress.json`
  - 保存内容包括：策略结果、测试用例、所有策略列表、配置信息
  - 刷新网页或关闭浏览器不会丢失进度
- **进度恢复**：
  - 检测到已保存的进度时，自动判断是否完成
  - 如果完成：显示完成提示，引导用户查看历史结果
  - 如果未完成：提供"继续压测"按钮，从中断处继续
- **实现位置**：
  - `_save_benchmark_progress()`：保存进度
  - `_load_benchmark_progress()`：加载进度
  - `_clear_benchmark_progress()`：清除进度

##### 历史记录管理
- **历史记录保存**：
  - 每次压测完成后自动保存到 `logs/benchmark_history.jsonl`
  - 保存内容包括：策略结果、最终指标、运行名称、备注、时间戳等
  - 使用JSONL格式，便于追加和读取
- **历史记录查看**：
  - 独立的"📈 历史结果"标签页，与"▶️ 运行压测"完全分离
  - 下拉选择历史记录，显示完整的排行榜和可视化分析
  - 显示运行名称、备注、测试集信息等
- **实现位置**：
  - `_save_benchmark_history()`：保存历史记录
  - `_load_benchmark_history()`：加载历史记录
  - `_render_benchmark_history()`：渲染历史记录界面

##### 运行名称和备注功能
- **运行名称**：
  - 默认值为当前时间（格式：`YYYY-MM-DD HH:MM:SS`）
  - 用户可自定义名称，便于识别不同的测试运行
- **更新备注**：
  - 文本区域，用于记录本次测试的更新要点或改动说明
  - 保存到历史记录中，可在历史结果中查看
- **实现位置**：`_render_benchmark_run()` 函数中的输入框

##### 排行榜和评分系统优化
- **排名显示**：
  - 添加中文排名列（第一名、第二名、第三名等）
  - 按综合评分从高到低排序
- **Benchmark高亮**：
  - 仅单独使用Benchmark模型的行显示橙色背景
  - 双模型组合中的Benchmark不特殊标记
- **评分规则说明**：
  - 添加"评分规则说明"展开器，详细说明综合评分的计算公式
  - 添加"详细评分分解"展开器，显示每个策略的详细评分计算过程
- **实现位置**：`_display_leaderboard()` 函数

##### 可视化分析增强
- **修复标题显示**：
  - 所有图表标题添加 `pad=20` 参数，确保标题可见
  - 添加Y轴标签，提高可读性
- **倒序展示**：
  - 综合评分：按评分从高到低排序（最高在最上面）
  - 成本分析：按成本从低到高排序（最低在最上面）
  - 延迟分析：按延迟从低到高排序（最低在最上面）
  - 质量分析：按质量从高到低排序（最高在最上面）
  - 路由准确率：按准确率从高到低排序（最高在最上面）
- **运行时间可视化**：
  - 新增"运行时间分析"标签页
  - 包含平均生成时长和平均评估时长两个图表
  - 按时间从短到长排序（最快的在最上面）
- **实现位置**：`_display_leaderboard()` 函数中的可视化部分

##### UI优化
- **紧凑显示**：
  - 测试集路径和测试用例信息使用更紧凑的布局
  - 测试组合信息使用 `st.caption()` 显示，节省空间
- **功能分离**：
  - "运行压测"标签页：仅用于运行新测试或继续未完成的测试
  - "历史结果"标签页：仅用于查看已完成的测试结果和可视化分析
  - 测试完成后，结果只在历史结果中显示，不在运行压测中显示

##### 技术实现细节
- 使用 `st.session_state` 保存运行名称和备注
- 使用 `st.empty()` 和 `st.container()` 实现动态更新
- 使用 `pandas.DataFrame` 和 `df.style.apply()` 实现表格高亮
- 使用 `matplotlib` 实现可视化图表
- 所有时间单位统一为秒，显示格式保留2-3位小数

---

### [update3.8] - 2026-03-08
...
- **模型选择器功能**：在Chat界面侧边栏添加模型选择器，支持用户选择不同的大模型用于RAG最终生成环节
  - 支持模型列表：
    ...
    - api-glm-4-plus（通过智增增代理的GLM API）
  - Benchmark模型自动标注为[Benchmark]
  - 使用ModelManager统一管理模型配置和切换
  - 实现位置：`src/observability/dashboard/pages/chat_interface.py`

#### 运行时间展示功能
- **Chat界面运行时间**：展示从用户询问到最终页面输出答案的总时长（单位：秒，保留2位小数）
  - 实现位置：Chat界面答案下方，使用`st.caption()`显示
- **LLM Arena单次Query时间展示**：
  - 生成时长：记录从开始执行RAG查询到生成答案的时间
  - 测评时长：记录Ragas评测的时间（包括faithfulness、answer_relevancy、context_precision三个维度）
  - 实现位置：关键指标可视化之前，使用两列布局展示
- **LLM Arena批量评估时间展示**：
  - 平均生成时长：在排行榜表格中展示每条query的平均生成时间
  - 平均测评时长：在排行榜表格中展示每条query的平均测评时间
  - 实现位置：`_display_leaderboard`函数中的排行榜表格
  - 数据模型：在`StrategyMetrics`中添加了`avg_eval_time_s`字段

#### 技术实现细节
- 时间记录使用`time.monotonic()`确保准确性
- 测评失败时默认时间为0.0，确保系统稳定性
- 所有时间单位统一为秒，显示格式保留2-3位小数
...
- Transform & Enrichment（结构转换与深度增强）
	...
	- **核心增强策略**：
		...
		4. **文档级意图标注 (Document-Level Intent Tagging)** [新增]：
			- 策略：复用 Query 侧的本地意图分类模型（TF‑IDF + XGBoost），在摄取时对整篇文档做一次高层意图识别。
			- 产出：为每篇文档生成统一的业务意图标签（如 `returns` / `fabric_care` / `styling` / `escalation` / `chitchat`），写入所有 Chunk 的 `metadata.doc_intent` 字段。
			- 物理视图：根据 `doc_intent` 将原始文档复制到按意图划分的文件库目录（`data/documents_by_intent/{intent}/{collection}/`），形成自然可见的意图分库。
...
- **Identify the real query requirement  （识别是否是垃圾信息）** [新增/重构]
	- **核心假设**：在高端服饰 CRM 场景中，流量往往混杂着无意义的闲聊/恶意试探与高价值的售后/穿搭咨询。系统需要兼顾“极致的防御性能”与“极高的业务分类精度”。
	- **Layer 1: Spam/Safety Gateway (前置垃圾与安全拦截)**：
		- **位置**：在任何 NLP 预处理之前，直接面对用户的 Raw Query。
		- **技术选型**：极轻量级的二分类器（如 FastText 或 基于规则的正则表达式 + 简单线性分类器）。
		- **职责**：快速识别并丢弃纯标点、无意义乱码、明显的恶意攻击或纯闲聊。将这部分低价值流量直接短路，返回标准安抚话术，绝不消耗后续的 NLP 分词与大模型算力。
...
- **Fine-Grained Intent Classifier (细粒度业务意图分类)**：
		- **位置**：在 Query Processor 提取出干净的特征之后。
		- **技术选型**：本地离线训练的 `TF-IDF + XGBoost / Random Forest` 多分类模型。受益于上游清晰的特征，该模型能在较少样本下达到极高准确率。
		- **细分意图与路由策略**：
			1. **售后与退换 (Returns)**：进入 Hybrid Search 检索售后条款文档。
			2. **面料与洗护 (Fabric Care)**：进入 Hybrid Search 检索商品说明书。
			3. **穿搭建议 (Styling Advice)**：进入 Hybrid Search 检索 Lookbook，同时在上下文中注入 `Target: Cross-sell` 标记，触发后续的二级营销。
			4. **情绪升级 (Escalation)**：识别到强烈负面情绪，立即短路 RAG，生成转交人工客服的指令。
      5. **无关与闲聊 (Unrelated/Chitchat)** [新增]：识别到与电商业务完全无关的闲聊或无意义文本，直接短路 RAG 检索，使用极其轻量的预设话术兜底（例如：“我是您的专属穿搭顾问，关于这件衣服您有什么想了解的吗？”），将话题重新引导回业务主线。
...
- **Filtering & Reranking (精确过滤与重排)**
	- **Metadata Filtering Strategy (通用过滤策略)**：
		...
		- Query Processing 阶段应将结构化约束解析为通用 `filters`（例如 `collection`/`doc_type`/`language`/`time_range`/`access_level`/`doc_intent` 等），其中 `doc_intent` 与摄取阶段的文档意图标注保持一致，用于按业务意图精确筛选文档子集（如仅查询退换货文档库）。
...
#### 3.2.6 查询复杂度分类能力 (Query Complexity Classifier) [new]

为支持"简单问题走小模型、复杂问题走大模型"的路由策略，新增本地查询复杂度分类能力，作为模型路由的前置判断信号。

- **运行时分类器**：`src/core/query_engine/query_complexity_classifier.py`
	- 输入：用户 Query 文本
	- 输出：`is_simple` / `is_complex` / `confidence` / `probability_simple` / `probability_complex`
	- 默认模型路径：`data/models/query_complexity_classifier.pkl`
	- 降级策略：模型缺失或加载失败时，默认按 complex 路由，确保主链路稳定可用

- **离线训练脚本**：`scripts/train_query_complexity_model.py`
	- 默认训练数据：`data/training/query_complexity_dataset.csv`
	- 默认模型产物：`data/models/query_complexity_classifier.pkl`
	- 支持分类器：`xgboost` / `logistic`
	- 设计原则：本地离线训练、本地推理，不引入额外在线依赖
...
#### 3.4.5 Dashboard 功能设计（八页面架构）

Dashboard 基于 Streamlit 构建多页面应用（`st.navigation`），当前提供 8 个页面（6 个管理页 + Chat + LLM Arena）：
...
**页面 7：Chat 界面 (Chat Interface) [new]**
- **客户端 RAG 闭环**：在 Dashboard 内提供 Chat 页面，前端直接调用 HybridSearch + Reranker + RAGGenerator，完成检索、重排与生成的完整 RAG 闭环（与 MCP 工具共用检索层，但生成发生在 UI 侧）[new]。
- **引用来源最小打扰展示**：模型回答区仅展示主内容，引用来源通过小号"📚 引用来源 (N)" 折叠块按需展开，内部展示 `[index] source (page) + snippet` 列表，避免大段引用信息干扰阅读[new]。
- **点击查看 Source 详情**：当 source 为 URL 时，以可点击链接形式展示，支持在新标签页打开原文；本地路径则以纯文本展示，方便复制或在本机定位文档[new]。
- **会话历史持久化**：Chat 页面会话数据（标题 + 消息列表）序列化保存到本地 `data/chat/conversations.json` 文件，Dashboard 重启后自动加载，支持在侧边栏切换历史对话并恢复上下文[new]。
- **模型选择器功能[update3.8]**：在侧边栏添加模型选择器，支持用户选择不同的大模型用于RAG最终生成环节。支持7种模型（包括Benchmark模型自动标注），使用ModelManager统一管理模型配置和切换。
- **运行时间展示[update3.8]**：在答案下方展示从用户询问到最终输出答案的总运行时间（单位：秒，保留2位小数）。

**页面 8：LLM Arena (Model Benchmark) [update3.8]**
- **双模式评测**：提供单次互动测试（Interactive Playground）与批量压测（Exhaustive Benchmark）。
- **动态排行榜**：按综合评分展示策略排名，支持成本/质量/延迟/路由准确率等维度可视化。
- **进度恢复与历史记录**：批量压测支持进度持久化与恢复；历史结果以 JSONL 持久化，支持回看。
- **时间指标增强**：展示单次 Query 的生成/测评时长与批量评测平均时长（含 `avg_eval_time_s`）。
...
│   ├── chat_interface.py     # 页面 7：Chat 界面（客户端 RAG + 会话持久化）[new]
│   └── llm_arena.py          # 页面 8：LLM Arena（模型压测与排行榜）[update3.8]
└── services/
    ...
    └── scoring_engine.py     # 评分引擎（策略综合评分与排行榜字段）[update3.8]
```
...
### 4.6 工程诊断与联调脚本（Diagnostics & Verification）[new]

以下脚本用于环境诊断、模型可用性核验与联调验证（不替代正式 `pytest` 测试）：

- `python scripts/check_llm_arena_models.py`
  - 检查本地 Ollama 模型是否已下载，以及 API 模型配置是否齐备
- `python scripts/test_llm_arena.py`
  - 验证模型注册、路由逻辑与基础调用链路
- `python scripts/verify_model_update.py`
  - 校验旧模型引用是否已清理并切换到新模型组合
- `python scripts/check_deepseek_connection.py`
  - 校验主链路 DeepSeek/代理连接与基础调用可用性
- `python scripts/check_deepseek_arena.py`
  - 校验 LLM Arena 场景下 DeepSeek 模型连接状态
- `python scripts/test_ragas_eval.py`
  - 校验 `EVAL_API_KEY`/`EVAL_BASE_URL` 与 RAGAS 调用链路
...
│  │  ┌─────────────────────────────────────────────────────────────────────────────┐    │    │
│  │  │                 Layer 1: Spam/Safety Gateway (前置垃圾与安全拦截)              │    │    │
│  │  │               FastText / 规则引擎 | 拦截闲聊、乱码与恶意攻击 | 阻断返回           │    │    │
│  │  └──────────────────────────────────────┬──────────────────────────────────────┘    │    │
│  │                                         │ (非垃圾 Query)                            │    │
│  │  ┌──────────────────────────────────────▼──────────────────────────────────────┐    │    │
...
│  │                                         │ (特征 Token)                              │    │
│  │  ┌──────────────────────────────────────▼──────────────────────────────────────┐    │    │
│  │  │            Layer 2: Fine-Grained Intent Classifier (细粒度意图分类)            │    │    │
│  │  │              TF-IDF + XGBoost | 打标: 售后/洗护/穿搭/投诉 等意图               │    │    │
│  │  └─────────────────────────────────────────────────────────────────────────────┘    │    │
│  │                                       │                                             │    │
│  │  ┌────────────────────────────────────┼────────────────────────────────────┐        │    │
...
│   │   ├── query_engine/                # 查询引擎模块
│   │   │   ├── __init__.py
│   │   │   ├── intent_router.py         # [新增] 双层意图路由与模型推理网关
│   │   │   ├── query_processor.py       # [需修改] 整合 intent_router
...
│
├── data/                                # 数据目录
│   ├── models/                          # [新增] 存放离线训练的分类模型 (.pkl)
│   ├── documents/                       # 原始文档存放
│   │   └── {collection}/                # 按集合分类
│   ├── documents_by_intent/             # 按业务意图划分的文档库视图 [新增]
│   │   └── {intent}/{collection}/       # 例如 returns/default/, fabric_care/default/ 等
│   ├── images/                          # 提取的图片存放
...
│
├── scripts/                             # 脚本目录
│   ├── train_spam_model.py              # [新增] Layer 1 垃圾流量拦截器训练脚本
│   ├── train_intent_model.py            # [新增] Layer 2 细粒度意图分类器训练脚本
...
| `types.py` | 核心数据类型/契约（全链路复用） | 定义 `Document/Chunk/ChunkRecord/ProcessedQuery/RetrievalResult`；序列化稳定；作为 ingestion/retrieval/mcp 的数据契约中心 ;IntentType 枚举，为 ProcessedQuery 增加意图与拦截标记。[新增]|
| `intent_router.py` | [新增]双层意图路由 | 加载 data/models/ 下的本地模型，执行 Layer 1 (垃圾拦截) 与 Layer 2 (业务意图细分)，输出分类结果与置信度。|
...
| `dashboard/pages/chat_interface.py` | Chat 界面 | 客户端 RAG 闭环、引用折叠展示、会话持久化、模型选择器 |
| `dashboard/pages/llm_arena.py` | LLM Arena | 单次/批量评测、动态排行榜、进度恢复、历史记录 |
...
| `dashboard/services/scoring_engine.py` | 评分引擎 | 综合评分计算与排行榜指标聚合（含时长维度） |
...
#### 5.5.1 当前实现字段对齐说明（与 `config/settings.yaml` 同步）[new]

当前代码实现中，以下键名为有效字段，文档与示例应优先使用这些命名：

- `vector_store.provider`、`vector_store.persist_directory`
- `retrieval.dense_top_k`、`retrieval.sparse_top_k`、`retrieval.fusion_top_k`、`retrieval.rrf_k`
- `rerank.provider`、`rerank.enabled`、`rerank.top_k`
- `observability.trace_enabled`、`observability.trace_file`、`observability.structured_logging`
...
7. **阶段 G：可视化管理平台 Dashboard**
   - 目的：搭建 Streamlit 多页面管理平台（系统总览 / Chat / 数据浏览 / Ingestion 管理 / Ingestion 追踪 / Query 追踪 / 评估面板 / LLM Arena），实现 DocumentManager 跨存储协调。
8. **阶段 H：评估体系**
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| **D0-a** | **Layer 1: 垃圾流量拦截器离线训练** | [x] | 2026-03-05 | `scripts/train_spam_model.py` 构建二分类器并序列化至 `data/models/spam_gateway.pkl`。 |
| **D0-b** | **Layer 2: 细粒度意图模型离线训练** | [x] | 2026-03-05 | `scripts/train_intent_model.py` 构建 TF-IDF+XGBoost 多分类器并序列化至 `data/models/intent_classifier.pkl`。 |
| **D0-c** | **双层意图路由网关 (Intent Router) 实现** | [x] | 2026-03-05 | `src/core/query_engine/intent_router.py` 加载上述两个本地模型；`src/core/types.py` 中的 `ProcessedQuery` 增加 `intent` 和 `is_spam` 字段。 |
| D1 | QueryProcessor（关键词提取 + filters） | [x] | 2026-03-05 | ProcessedQuery类型+关键词提取+停用词过滤+filter语法+38单元测试；**[新增] 内部串联 IntentRouter 实现双层分流**，补充相应路由逻辑的单元测试。 |
...
| G4 | Ingestion 管理页面 | [x] | 2026-02-09 | 文件上传+IngestionPipeline集成+实时进度条+TraceContext自动记录；[新增] 文档级意图预测与人工确认、逻辑 collection 标签及移动、all 视图与 Dashboard 级 collection 覆盖 |
...
| G7 | Chat 界面与引用来源 UX 优化[new] | [x] | 2026-03-06 | 新增 Chat 页面；在 UI 侧完成检索+重排+生成的 RAG 闭环，[new] 将引用来源改为小号"📚 引用来源 (N)" 折叠展示，并支持点击查看完整 source 列表与 snippet；[new] 聊天历史持久化到 `data/chat/conversations.json`，Dashboard 重启后可恢复对话 |
| G8 | Chat界面模型选择器与运行时间展示[update3.8] | [x] | 2026-03-08 | 在Chat界面侧边栏添加模型选择器，支持7种模型切换（包括Benchmark标注）；添加总运行时间展示（从询问到输出答案）；在LLM Arena中添加生成时长和测评时长展示（单次和批量评估） |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| I1 | E2E：MCP Client 侧调用模拟 | [x] | 2026-03-01 | tests/e2e/test_mcp_client.py 子进程 stdio + tools/list + tools/call，query_knowledge_hub 返回 content/citations |
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-03-01 | tests/e2e/test_dashboard_smoke.py + dashboard_smoke_runner.py，AppTest 多页冒烟（当前为 8 页） |
| I3 | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-03-01 | 快速开始/配置说明/MCP 示例/Dashboard 指南/运行测试/常见问题 |
| I4 | 清理接口一致性（契约测试补齐） | [x] | 2026-03-01 | delete_by_metadata 边界 + reranker/evaluator 契约与边界 |
| I5 | 全链路 E2E 验收 | [x] | 2026-03-01 | 验收步骤已文档化于 README；单元/契约/E2E(MCP+Dashboard) 测试通过；全链路需配置后按 README 执行 |
...
## 阶段 G：可视化管理平台 Dashboard（目标：八页面完整可视化管理）
...
### G7：Chat 界面与引用来源 UX 优化[new]
- **目标**：实现 Dashboard Chat 页面，提供客户端 RAG 闭环体验，优化引用来源展示与会话持久化。
- **前置依赖**：D5（HybridSearch）、D6（Reranker）、G1（Dashboard 架构）
- **修改文件**：
  - `src/observability/dashboard/pages/chat_interface.py`（新增）
- **实现要点**：
  - 客户端 RAG 闭环：在 UI 侧完成检索+重排+生成的完整 RAG 闭环
  - 引用来源折叠展示：小号"📚 引用来源 (N)" 折叠块，避免干扰阅读
  - 会话历史持久化：保存到 `data/chat/conversations.json`，支持历史对话恢复
- **验收标准**：可在 Dashboard 中发起对话，查看引用来源，重启后恢复历史对话。
- **测试方法**：手动验证 Chat 页面功能。

### G8：Chat界面模型选择器与运行时间展示[update3.8]
- **目标**：在Chat界面添加模型选择器功能，支持用户选择不同的大模型；添加运行时间展示功能。
- **前置依赖**：G7（Chat 界面）、B阶段（ModelManager）
- **修改文件**：
  - `src/observability/dashboard/pages/chat_interface.py`（增强：模型选择器、运行时间展示）
  - `src/observability/dashboard/pages/llm_arena.py`（增强：运行时间展示）
  - `src/observability/dashboard/services/scoring_engine.py`（增强：StrategyMetrics添加avg_eval_time_s字段）
- **实现要点**：
  - 模型选择器：在侧边栏添加模型选择器，支持7种模型切换（包括Benchmark模型自动标注）
  - Chat界面运行时间：展示从用户询问到最终输出答案的总时长（单位：秒）
  - LLM Arena单次Query时间：展示生成时长和测评时长（Ragas三个维度）
  - LLM Arena批量评估时间：在排行榜中展示平均生成时长和平均测评时长
- **验收标准**：
  - Chat界面可选择模型并看到运行时间
  - LLM Arena单次和批量评估都能看到相应的时间展示
- **测试方法**：手动验证模型选择和时间展示功能。
...
- **M5（完成阶段 G）**：八页面可视化管理平台就绪（含 Chat 与 LLM Arena），数据可浏览、可管理、链路可追踪。
