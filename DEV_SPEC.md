...
Dashboard 基于 Streamlit 构建多页面应用（`st.navigation`），提供六大功能页面；此外补充一个面向终端用户的 Chat 体验页面，用于完整演示 RAG 闭环与引用来源展示[new]：
...
**附加页面：Chat 界面 (Chat Interface)[new]**
...
│   └── chat_interface.py     # Chat 界面：客户端 RAG + 引用来源折叠展示 + 会话持久化[new]
└── services/
...
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-03-01 | tests/e2e/test_dashboard_smoke.py + dashboard_smoke_runner.py，AppTest 六页冒烟 |
