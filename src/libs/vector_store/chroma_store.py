...
def _create_chroma_client(persist_path: Path) -> Any:
    """Create a ChromaDB PersistentClient with compatibility handling.
    
    This helper function handles ChromaDB version compatibility issues,
    particularly the 'RustBindingsAPI' object has no attribute 'bindings' error
    that can occur with certain ChromaDB versions on Windows.
    ...
    Raises:
        ...
        RuntimeError: If client initialization fails after all attempts.
    """
    ...
    persist_path = Path(persist_path).resolve()
    ...
    try:
        ...
        # Some versions have issues with tenant/database validation
        if "bindings" in str(e) or "tenant" in str(e).lower():
            logger.warning(
                f"ChromaDB initialization issue detected: {e}. "
                "Attempting alternative initialization method..."
            )
            try:
                # Try without explicit settings (use defaults)
                return chromadb.PersistentClient(path=str(persist_path))
            except Exception as e2:
                logger.warning(
                    f"Alternative initialization also failed: {e2}. "
                    "This might indicate a corrupted ChromaDB database. "
                    "Consider deleting the database directory and re-ingesting documents."
                )
                raise RuntimeError(
                    f"Failed to initialize ChromaDB client at '{persist_path}': {e2}. "
                    "If this persists, try deleting the database directory and re-ingesting."
                ) from e2
        else:
            raise RuntimeError(
...
class ChromaStore(BaseVectorStore):
    ...
    def __init__(self, settings: Settings, **kwargs: Any) -> None:
        ...
        if not CHROMADB_AVAILABLE:
            raise ImportError(
                "ChromaDB 包未安装。ChromaDB 是本地向量数据库，无需部署，但需要安装 Python 包。\n"
                "请运行以下命令安装：\n"
        ...
        try:
            self.client = _create_chroma_client(self.persist_directory)
        except Exception as e:
            raise RuntimeError(
