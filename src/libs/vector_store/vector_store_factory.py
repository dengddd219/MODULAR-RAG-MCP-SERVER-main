...
# ---------------------------------------------------------------------------
# NOTE:
# Some call sites import the factory directly via
# ``from src.libs.vector_store.vector_store_factory import VectorStoreFactory``
# instead of importing the package ``src.libs.vector_store`` (whose
# ``__init__`` also performs registration). To ensure providers are
# available in both cases, we eagerly register built‑in providers here.
try:  # pragma: no cover - simple import/registration glue
    from src.libs.vector_store.chroma_store import ChromaStore

    VectorStoreFactory.register_provider("chroma", ChromaStore)
except Exception:
    # If chromadb or its dependencies are not installed, we silently skip
    # registration. The factory will then report that no providers are
    # available, along with guidance in the error message.
    pass
