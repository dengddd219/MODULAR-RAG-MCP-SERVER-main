...
class OpenAILLM(BaseLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # API key: explicit parameter > settings.llm.api_key > env var
        self.api_key = api_key or getattr(settings.llm, "api_key", None) or os.environ.get("OPENAI_API_KEY")
        if not self.api_key:
            raise ValueError(
                "OpenAI API key not provided. Configure 'llm.api_key' in settings.yaml, "
                "set OPENAI_API_KEY environment variable, or pass api_key parameter."
            )
        
        # Base URL: explicit parameter > settings.llm.base_url > default
        if base_url:
