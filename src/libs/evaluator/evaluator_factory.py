...
class EvaluatorFactory:
    ...
    def create(cls, settings: Settings, **override_kwargs: Any) -> BaseEvaluator:
        ...
        """
        try:
            # 【终极修复】使用 getattr 安全获取。
            # 如果 settings 里有 'evaluation' 属性，就取出来；如果没有，就直接把 settings 自己赋给它。
            evaluation_settings = getattr(settings, 'evaluation', settings)
            
            if evaluation_settings is None:
                raise AttributeError("evaluation settings is None")
