...
# Module-level cache for settings (keyed by path and mtime)
_settings_cache: Dict[tuple[Path, float], Settings] = {}


def load_settings(path: str | Path | None = None) -> Settings:
    """Load settings from a YAML file and validate required fields.
    
    Settings are cached based on file path and modification time.
    If the file is modified, the cache is automatically invalidated.
    ...
    # Check cache using path and modification time
    mtime = settings_path.stat().st_mtime
    cache_key = (settings_path, mtime)
    ...
    # Cache the result (keep only the most recent entry per path)
    _settings_cache.clear()  # Simple strategy: clear old entries
