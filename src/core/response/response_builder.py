...
class ResponseBuilder:
    ...
    def build(
        ...
        Args:
            ...
            collection_count: Optional number of documents in the collection.
                Used to distinguish between empty collection vs no query matches.
        ...
        if not results:
            return self._build_empty_response(query, collection, collection_count)
    ...
    def _build_empty_response(
        ...
        Args:
            ...
            collection: Optional collection name.
            collection_count: Optional number of documents in the collection.
                If None, we cannot determine if collection is empty or query didn't match.
            
        Returns:
            MCPToolResponse indicating no results found.
        """
        # Build bilingual response (English + Chinese) for better Claude understanding
        content = "## No Results Found / 未找到相关结果\n\n"
        content += f"**Query / 查询:** {query}\n\n"
        
        # Check if collection is empty or query didn't match
        if collection_count is not None:
            if collection_count == 0:
                # Collection is empty - no documents uploaded
                if collection:
                    content += f"⚠️ **The collection `{collection}` is empty** - no documents have been uploaded yet.\n\n"
                    content += f"⚠️ **集合 `{collection}` 为空** - 尚未上传任何文档。\n\n"
                else:
                    content += "⚠️ **The knowledge base is empty** - no documents have been uploaded yet.\n\n"
                    content += "⚠️ **知识库为空** - 尚未上传任何文档。\n\n"
                content += "**To use RAG, you need to:**\n"
                content += "1. Upload relevant documents (PDFs, articles, notes, etc.) to your knowledge base\n"
                content += "2. Wait for the documents to be indexed\n"
                content += "3. Then query the knowledge base\n\n"
                content += "**要使用 RAG，您需要：**\n"
                content += "1. 上传相关文档（PDF、文章、笔记等）到知识库\n"
                content += "2. 等待文档被索引\n"
                content += "3. 然后查询知识库\n"
            else:
                # Collection has documents but query didn't match
                if collection:
                    content += f"The collection `{collection}` contains {collection_count} documents, but no results matched your query.\n\n"
                    content += f"集合 `{collection}` 包含 {collection_count} 个文档，但查询未找到匹配结果。\n\n"
                else:
                    content += f"The knowledge base contains {collection_count} documents, but no results matched your query.\n\n"
                    content += f"知识库包含 {collection_count} 个文档，但查询未找到匹配结果。\n\n"
                content += "**Suggestions / 建议:**\n"
                content += "- Try using different keywords or rephrasing your query\n"
                content += "- Check if the query terms match the document content\n"
                content += "- Expand search scope (e.g., don't specify a collection)\n\n"
                content += "- 尝试使用不同的关键词或重新表述查询\n"
                content += "- 检查查询词是否与文档内容匹配\n"
                content += "- 扩大搜索范围（例如，不指定集合）\n"
        else:
            # Cannot determine collection status
            if collection:
                content += f"No results found in collection `{collection}` for your query.\n\n"
                content += f"在集合 `{collection}` 中未找到与查询相关的文档。\n\n"
            else:
                content += "No results found in the knowledge base for your query.\n\n"
                content += "未找到与查询相关的文档。\n\n"
            content += "**Suggestions / 建议:**\n"
            content += "- Try using different keywords\n"
            content += "- Check if documents have been uploaded to the knowledge base\n"
            content += "- Expand search scope (e.g., don't specify a collection)\n\n"
            content += "- 尝试使用不同的关键词\n"
            content += "- 检查是否已摄取相关文档\n"
        ...
        if collection_count is not None:
            ...
            metadata["collection_empty"] = (collection_count == 0)
