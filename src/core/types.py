...
class IntentType(str, Enum):
    """High-level intent types for processed queries.
    
    This enum is used to annotate ProcessedQuery with routing information
    for downstream components (e.g., IntentRouter, HybridSearch).
    """
...
class ProcessedQuery:
    ...
    Attributes:
        ...
        intent: Optional high-level intent label for routing (from IntentRouter)
        is_spam: Whether the query is classified as low-value / spam traffic
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for serialization."""
        data = asdict(self)
        # Ensure IntentType is serialised as its string value
        intent_value = data.get("intent")
        if isinstance(intent_value, IntentType):
            data["intent"] = intent_value.value
        return data
