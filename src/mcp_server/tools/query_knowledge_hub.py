...
class QueryKnowledgeHubTool:
    ...
    async def execute(
        ...
        try:
            ...
            # Get collection count to provide better empty response message
            collection_count = await asyncio.to_thread(
                self._get_collection_count, effective_collection
            )
    ...
    def _get_collection_count(self, collection: str) -> Optional[int]:
        """Get the number of documents/chunks in a collection.
        
        Args:
            collection: Collection name.
            
        Returns:
            Number of documents in collection, or None if cannot determine.
        """
        try:
            from src.libs.vector_store.vector_store_factory import VectorStoreFactory
            
            # Create a temporary vector store to check collection count
            vector_store = VectorStoreFactory.create(
                self.settings,
                collection_name=collection,
            )
            
            # Check if it's a ChromaStore and has a count method
            if hasattr(vector_store, 'collection') and hasattr(vector_store.collection, 'count'):
                count = vector_store.collection.count()
                logger.debug(f"Collection '{collection}' has {count} documents")
                return count
            else:
                logger.debug(f"Cannot get count for collection '{collection}' - unsupported vector store type")
                return None
        except Exception as e:
            logger.warning(f"Failed to get collection count for '{collection}': {e}")
            return None
    ...
    def _build_error_response(
        ...
        if "chromadb" in error_lower or "import" in error_lower:
            content += "**错误原因:** ChromaDB Python 包未安装\n\n"
            content += "**解决方案:**\n"
            content += "1. 安装 ChromaDB 包：\n"
            content += "   ```bash\n"
            content += "   pip install chromadb\n"
            content += "   ```\n\n"
            content += "2. 或者重新安装项目所有依赖：\n"
            content += "   ```bash\n"
            content += "   pip install -e .\n"
            content += "   ```\n\n"
            content += "**注意:** ChromaDB 是本地向量数据库，无需额外部署服务。\n"
            content += "只需安装 Python 包即可使用。\n\n"
        elif "collection" in error_lower and "empty" in error_lower:
            content += "**错误原因:** 知识库集合为空，没有已索引的文档\n\n"
            content += "**解决方案:**\n"
            content += "1. 运行摄取脚本上传文档：\n"
            content += "   ```bash\n"
            content += "   python scripts/ingest.py --path <文档路径> --collection default\n"
            content += "   ```\n\n"
            content += "2. 或通过 Dashboard 上传文档\n\n"
        else:
