...
from src.ingestion.transform.document_intent_classifier import DocumentIntentClassifier
...
class IngestionPipeline:
    ...
    def __init__(
        ...
        # Stage 4d: Document-level intent classification (optional)
        self.document_intent_classifier = DocumentIntentClassifier()
        logger.info("  ✓ DocumentIntentClassifier initialized")
    ...
    def run(
        ...
        stages: Dict[str, Any] = {}
        _total_stages = 6
        doc_intent_value: Optional[str] = None
        doc_intent_confidence: Optional[float] = None
        doc_intent_distribution: Optional[Dict[str, float]] = None
        ...
        try:
            ...
            # 4d: Document Intent Classification (optional)
            logger.info("  4d. Document Intent Classification...")
            from src.ingestion.transform.document_intent_classifier import (  # local import to avoid cycles
                DocumentIntentPrediction,
            )

            doc_intent_prediction = self.document_intent_classifier.classify_document_with_scores(
                document, chunks
            )
            if doc_intent_prediction is not None:
                doc_intent_value = doc_intent_prediction.intent.value
                doc_intent_confidence = doc_intent_prediction.confidence
                doc_intent_distribution = doc_intent_prediction.distribution
                for c in chunks:
                    c.metadata["doc_intent"] = doc_intent_value
                logger.info(
                    "      Document intent: %s (confidence=%.4f)",
                    doc_intent_value,
                    doc_intent_confidence,
                )
            else:
                logger.info("      Document intent: <unknown / skipped>")
            ...
            stages["transform"] = {
                ...
                "document_intent": {
                    "value": doc_intent_value,
                    "confidence": doc_intent_confidence,
                    "distribution": doc_intent_distribution,
                },
            }
            ...
            logger.info(
                "  Dense vectors: %d (dim=%d)",
                len(dense_vectors),
                len(dense_vectors[0]) if dense_vectors else 0,
            )
            logger.info("  Sparse stats: %d documents", len(sparse_stats))
            ...
            # 6a: Vector Upsert
            if chunks and dense_vectors and len(dense_vectors) == len(chunks):
                logger.info("  6a. Vector Storage (ChromaDB)...")
                vector_ids = self.vector_upserter.upsert(chunks, dense_vectors, trace)
                logger.info("      Stored %d vectors", len(vector_ids))
            else:
                logger.warning(
                    "  6a. Skipping vector storage because chunk_count=%d, vector_count=%d",
                    len(chunks),
                    len(dense_vectors),
                )
            ...
            if sparse_stats:
                ...
                logger.info("      Index built for %d documents", len(sparse_stats))
            else:
                logger.warning("  6b. Skipping BM25 index build because sparse_stats is empty")
            ...
            if images:
                ...
                logger.info("      Indexed %d images", len(images))
            else:
                logger.info("  6c. Image Storage Index skipped (no images)")

            # Note: physical copy to intent-specific folders (documents_by_intent)
            # is now handled at the dashboard/UI layer after user confirmation
            # of the final intent label.
            stages["storage"] = {
                "vector_count": len(vector_ids),
                "bm25_docs": len(sparse_stats),
                "images_indexed": len(images),
                "doc_intent": doc_intent_value,
            }
