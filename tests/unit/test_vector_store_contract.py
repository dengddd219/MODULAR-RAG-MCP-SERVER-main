...
class FakeVectorStore(BaseVectorStore):
    ...
    def delete_by_metadata(
        ...
        """Delete records matching metadata filter (contract: same shape as ChromaStore)."""
        if not filter_dict:
            raise ValueError("filter_dict cannot be empty")
        to_remove = [
            rid
            for rid, rec in self.storage.items()
            if all(rec.get("metadata", {}).get(k) == v for k, v in filter_dict.items())
        ]
...
class TestDeleteByMetadataContract:
    """Contract tests for delete_by_metadata (boundary behavior).

    Any VectorStore implementing delete_by_metadata must: accept filter_dict
    and optional trace, return int (number deleted), raise ValueError for
    empty filter_dict.
    """

    def test_delete_by_metadata_empty_filter_raises(self) -> None:
        """Empty filter_dict must raise ValueError."""
        store = FakeVectorStore()
        with pytest.raises(ValueError, match="filter_dict cannot be empty"):
            store.delete_by_metadata({})

    def test_delete_by_metadata_no_match_returns_zero(self) -> None:
        """When no records match, return 0."""
        store = FakeVectorStore()
        store.upsert([
            {"id": "a", "vector": [0.1], "metadata": {"source": "x.pdf"}},
        ])
        n = store.delete_by_metadata({"source": "nonexistent.pdf"})
        assert n == 0
        assert len(store.storage) == 1

    def test_delete_by_metadata_match_returns_count(self) -> None:
        """When records match, return number deleted and remove them."""
        store = FakeVectorStore()
        store.upsert([
            {"id": "a", "vector": [0.1], "metadata": {"doc_hash": "h1"}},
            {"id": "b", "vector": [0.2], "metadata": {"doc_hash": "h2"}},
            {"id": "c", "vector": [0.3], "metadata": {"doc_hash": "h1"}},
        ])
        n = store.delete_by_metadata({"doc_hash": "h1"})
        assert n == 2
        assert "a" not in store.storage and "c" not in store.storage
        assert "b" in store.storage

    def test_delete_by_metadata_return_type_int(self) -> None:
        """Return value must be int."""
        store = FakeVectorStore()
        store.upsert([{"id": "x", "vector": [0.1], "metadata": {"k": "v"}}])
        n = store.delete_by_metadata({"k": "v"})
