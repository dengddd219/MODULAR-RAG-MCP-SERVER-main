...
class TestResponseBuilder:
    ...
    def test_build_empty_response(
        ...
        assert "No Results Found" in response.content or "未找到相关结果" in response.content
        assert "Suggestions" in response.content or "建议" in response.content
    
    def test_build_empty_response_with_empty_collection(
        self,
        response_builder: ResponseBuilder,
        empty_retrieval_results: List[RetrievalResult],
    ) -> None:
        """Test response building with empty collection (collection_count=0)."""
        response = response_builder.build(
            results=empty_retrieval_results,
            query="test query",
            collection="docs",
        ...
        assert response.is_empty is True
        assert len(response.citations) == 0
        assert "empty" in response.content.lower() or "为空" in response.content
        assert response.metadata.get("collection_empty") is True
        assert response.metadata.get("collection_count") == 0
    
    def test_build_empty_response_with_non_empty_collection(
        self,
        response_builder: ResponseBuilder,
        empty_retrieval_results: List[RetrievalResult],
    ) -> None:
        """Test response building when collection has docs but query doesn't match."""
        response = response_builder.build(
            results=empty_retrieval_results,
            query="test query",
            collection="docs",
        ...
        assert response.metadata.get("collection_empty") is False
        assert response.metadata.get("collection_count") == 100
