...
class TestRerankerFactory:
    ...
    def test_rerank_return_shape_contract(self) -> None:
        """rerank() must return a list of dicts (same or subset of candidates)."""
        RerankerFactory.register_provider("fake", FakeReranker)
        settings = MagicMock()
        settings.rerank = MagicMock()
        settings.rerank.enabled = True
        settings.rerank.provider = "fake"
        reranker = RerankerFactory.create(settings)
        candidates = [{"id": "a", "score": 0.1}, {"id": "b", "score": 0.9}]
        result = reranker.rerank("query", candidates)
        assert isinstance(result, list)
        assert len(result) == 2
        assert all(isinstance(r, dict) and "id" in r for r in result)

    def test_create_passes_override_kwargs(self) -> None:
        """Factory create() should pass override_kwargs to provider constructor."""
        RerankerFactory.register_provider("fake", FakeReranker)
        settings = MagicMock()
        settings.rerank = MagicMock()
        settings.rerank.enabled = True
        settings.rerank.provider = "fake"
        reranker = RerankerFactory.create(settings, extra="value")
        assert getattr(reranker, "kwargs", {}).get("extra") == "value"
